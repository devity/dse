...
$vars['DGCG']['Program']['Image']['FileName']="";
			...
			if($L && !$Done){
				...
				switch($La[0]){
					...
					case 'hole-oval-outline':						
						...
						$WidthX=$La[7];
						$WidthY=$La[8];
						dgcg_hole_oval($X, $Y, $Z, $DiameterX, $DiameterY, $Depth, $WidthX, $WidthY);
						break;
					case 'arc':						
						if($La[1][0]=="+"){
							$X+=substr($La[1],1);
...
function dgcg_hole($x,$y,$z,$Diameter,$Depth){
	...
	while( $PointsOnPerimeter<$LinesSegmentsPerPerimeterApproximation*$Pi && $CurrentHoleRadius <= ($Diameter/2) ){		dpv(6," while($CurrentHoleRadius<$Diameter/2){\n");
...
function dgcg_hole_oval($x,$y,$z,$DiameterX,$DiameterY,$Depth,$WidthOuterX=0,$WidthOuterY=0){
	global $vars;
	...
	$CurrentHoleRadiusX=$vars['DGCG']['Tool']['Diameter']/2;
	$CurrentHoleRadiusY=$vars['DGCG']['Tool']['Diameter']/2;
	if($WidthOuterX){		
	...
	while(!$Done){
		...
		if($PointsOnPerimeter> ($LinesSegmentsPerPerimeterApproximation*3) ){
			$Done=TRUE;
		}
